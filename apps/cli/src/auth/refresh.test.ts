...
/**
 * `resolveServerUrl` is what this module actually calls. The factory used to export only
 * `loadSettings`, so every test that got as far as a refresh died on the missing export — seven
 * of them, unnoticed, because `check` only runs the tests belonging to the files being changed.
 */
vi.mock('../settings', () => ({
  resolveServerUrl: vi.fn(() => 'https://app.lobehub.com'),
}));
