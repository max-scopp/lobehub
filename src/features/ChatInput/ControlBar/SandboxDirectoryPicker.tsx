...
interface SandboxDirectoryPickerProps {
  /** Replaces the default chip, so this can also sit inside another menu. */
  children?: ReactNode;
  /**
 ...
 */
const SandboxDirectoryPicker = memo<SandboxDirectoryPickerProps>(
  ({ children, onChange, topicId, value }) => {
    ...
    const newFolderPath =
      newFolderName && !newFolderName.includes('/')
        ? joinPath(browsePath, newFolderName)
        : undefined;
